/*
Original task - https://www.codewars.com/kata/a-rule-of-divisibility-by-7
...
an array of numbers, the first being the last number m with at most 2 digits obtained by your function
...
class DivSeven {
    public static long[] seven(long m) {
        ...
        while (length > 2) {
            value = Long.parseLong(valueStr.substring(0, length-1)) - 2 * Long.parseLong(valueStr.substring(length-1));
            valueStr = value + "";
            length = valueStr.length();
            steps++;
        }

        return new long[]{value, steps};
    }
}