/*
Original task - https://www.codewars.com/kata/functions-of-integers-on-cartesian-plane
...
1	-	1	1	1	1	1	1
0	-	-	-	-	-	-	-
The task is to calculate the sum of f(x, y), g(x, y) and h(x, y)
for all integers x and y such that (1 <= x <= n, 1 <= y <= n).

The function sumin (sum of f) will take n as a parameter and return the sum of min(x, y)
in the domain 1 <= x <= n, 1 <= y <= n.
...
public class Funcij {
    ...
    public static BigInteger sumin(int n) {
        ...
        int currentN = n * 2 - 1;
        for (int i = 1; i <= n; i++) {
            sum = sum.add(new BigInteger(i * currentN + ""));
            currentN -= 2;
        }
