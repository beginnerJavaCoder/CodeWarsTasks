/*
Original task - https://www.codewars.com/kata/statistics-for-an-athletic-association
...
Median :
In statistics, the median is the number separating the higher half of a data sample from the lower half.
...
public class Stat {
    ...
    public static String stat(String strg) {
        ...
        int[] values = parseString(strg);
        Arrays.sort(values);

        return String.format("Range: %s Average: %s Median: %s",
                resultToString(range(values)),
                resultToString(mean(values)),
                resultToString(median(values)));
    }

    private static int[] parseString(String source) {
        String[] hms = source.split(", ");
        int[] seconds = new int[hms.length];
        for (int i = 0; i < hms.length; i++) {
            String[] tmp = hms[i].split("\\|");
            seconds[i] += Integer.parseInt(tmp[0]) * 3600;
            seconds[i] += Integer.parseInt(tmp[1]) * 60;
            seconds[i] += Integer.parseInt(tmp[2]);
        }

        return seconds;
    }

    private static double range(int[] values) {
    ...
    private static String resultToString(double seconds) {
        ...
        seconds -= minutes * 60;
        String sSeconds = (int) seconds < 10 ? "0" + (int) seconds : "" + (int) seconds;
