/*
Original task - https://www.codewars.com/kata/codewars-style-ranking-system
...
The progress earned from the completed activity is relative
to what the user's current rank is compared to the rank of the activity.
...
public class User {
    ...
    private final static int[] possibleRanks =
            new int[] {-8, -7, -6, -5, -4, -3, -2, -1, 1, 2, 3, 4, 5, 6, 7, 8};
    private static final int bound = possibleRanks.length - 1;
    ...
    public User() {
        rankPosition = 0;
        rank = possibleRanks[rankPosition];
        progress = 0;
    }

    public void incProgress(int activityRank) {
        if(activityRank < -8 || activityRank > 8 || activityRank == 0)
            throw new IllegalArgumentException(activityRank + " is not allowed here.");

        int ranksDifference = activityRank - rank;
        int progressAmount;

        if(ranksDifference < -1 && (rank != 1 && activityRank != -1)) return;
        if(ranksDifference < 0) {
            progressAmount = 1;
        } else if(ranksDifference == 0) {
            progressAmount = 3;
        } else if(rank < 0 && activityRank > 0) {
            progressAmount = 10 * --ranksDifference * ranksDifference;
        } else progressAmount = 10 * ranksDifference * ranksDifference;


        if(rank < 8) progress += progressAmount;
        if(isRankedUp()) incRank();
    }

    private boolean isRankedUp() {
        return progress >= 100;
    }
    ...
    private void incRank() {
        ...
        if(rankPosition >= bound) {
            progress = 0;
            rank = possibleRanks[bound];
        } else {
            rank = possibleRanks[rankPosition];
        }
    }
